...
describe('Integration Tests', () => {
  ...
  describe('完整文件處理流程', () => {
    it('應該完整處理一個模擬的文件流程', async () => {
      ...
      const fileResult = await FileProcessingService.processUploadedFile(mockFile);
  ...
  describe('學習和改進循環', () => {
    it('應該通過用戶回饋改進建議準確度', () => {
      // 初始建議
      const initialSuggestion = SmartSuggestionsService.generateSuggestion('自定義欄位');
      ...
      // 再次生成建議
      const improvedSuggestion = SmartSuggestionsService.generateSuggestion('自定義欄位');
  ...
  describe('錯誤處理和恢復', () => {
    it('應該優雅處理 OCR 失敗', async () => {
      // 模擬 OCR 失敗的情況
      try {
        await OCRService.extractTextFromImage('invalid-image-url');
      } catch (error) {
    ...
    it('應該處理無效的文件輸入', async () => {
      ...
      const result = await FileProcessingService.processUploadedFile(invalidFile);
  ...
  describe('性能和規模', () => {
    it('應該處理大量欄位而不崩潰', () => {
      ...
      const suggestions = SmartSuggestionsService.generateBulkSuggestions(largeFieldSet);
  ...
  describe('跨模組整合', () => {
    it('應該整合所有服務的輸出', async () => {
      ...
      const mockOCRResults: OCRResult[] = [
        ...
        { text: '數量', confidence: 85, bbox: [10, 50, 50, 70] },
        { text: '單價', confidence: 88, bbox: [10, 90, 50, 110] },
        { text: '總金額', confidence: 92, bbox: [10, 130, 70, 150] }
      ];
