// TableTemplate Pro - 批量處理服務

import { FileUploadResult, BatchProcessItem, TemplateConfig, FieldArea } from '../types';
import { FileProcessingService } from './fileProcessing';
...
export class BatchProcessorService {
  ...
  static async processBatch(
    files: File[],
    onProgress?: (completed: number, total: number, currentItem?: BatchProcessItem) => void,
  ...
  private static async processSingleFile(file: File): Promise<{
    ...
    try {
      // 1. 文件處理
      const fileResult = await FileProcessingService.processUploadedFile(file);
      ...
      if (file.type.startsWith('image/') || fileResult.pdfPages) {
        ...
        if (fileResult.pdfPages && fileResult.pdfPages.length > 0) {
          // PDF 文件：處理每一頁
          for (const page of fileResult.pdfPages.slice(0, 3)) { // 最多處理前3頁
            const pageResults = await OCRService.extractTextFromCanvas(page.canvas);
            ocrResults.push(...pageResults.words);
          }
        } else if (file.type.startsWith('image/')) {
          // 圖片文件：直接 OCR
          const imageData = fileResult.file ? URL.createObjectURL(fileResult.file) : '';
          if (imageData) {
            const imageResults = await OCRService.extractTextFromImage(imageData);
            ocrResults = imageResults.words;
          }
        }
    ...
    for (const template of templates) {
      for (const itemData of data) {
        try {
          ...
          const result = {
            success: true,
            data: new Blob(['Mock generated file'], { type: 'application/pdf' })
          };
          results.push(result);
        } catch (error) {
          results.push({
