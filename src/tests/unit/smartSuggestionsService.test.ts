...
describe("SmartSuggestionsService", () => {
  beforeEach(async () => {
    ...
    await SmartSuggestionsService.initialize();
  });
  ...
  describe("generateBulkSuggestions", () => {
    it("應該批量生成建議", () => {
      ...
      const suggestions =
        SmartSuggestionsService.generateBulkSuggestions(fields);
  ...
  describe("學習算法", () => {
    ...
    it("應該根據上下文調整建議", () => {
      ...
      const suggestionWithoutContext =
        SmartSuggestionsService.generateSuggestion("金額");

      // 帶上下文的建議應該更有信心
      expect(suggestionWithContext.confidence).toBeGreaterThanOrEqual(
        suggestionWithoutContext.confidence
      );
    });
  });
});